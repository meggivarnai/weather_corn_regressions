...
#combining data
iowa_2018 <- inner_join(iowa_temp2018,yields2018)
tibble(iowa_2018)
...
#plot
ggplot(iowa_2018,aes(x=meantmax, y=yield))+
  geom_point()
...
```{r}
# parameters to query 
params2 <- list(commodity_desc = "SOYBEANS", util_practice_desc = "GRAIN", prodn_practice_desc = "ALL PRODUCTION PRACTICES", year__GE = 1981, state_alpha = "IA")

# downloading the data (using nathan's code)
soybean_yields_all <- nassqs_yields(params2)

soybean_yields_all$county_ansi <- as.numeric(cornyieldsall$county_ansi)
soybean_yields_all$yield <- as.numeric(cornyieldsall$Value)

# clean and filter dataset
soybean_yields <- select(cornyieldsall, county_ansi, county_name, yield, year) %>%
  filter(!is.na(county_ansi) & !is.na(yield))
soybean_yields <- tibble(cornyields)
